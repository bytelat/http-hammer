...
impl ConnPool {
    pub fn new(upstreams: &[String], keep_alive: usize) -> Self {
        let epoll_fd =
            epoll_create1(EpollCreateFlags::EPOLL_CLOEXEC).expect("epoll_create1 failed");
        ...
        for up in upstreams {
            for _ in 0..keep_alive {
                ...
                let mut event = EpollEvent::new(EpollFlags::EPOLLIN, idx as u64);
        ...
        for i in 0..n {
            ...
            match (done, closed) {
                (true, false) => {
                    ...
                    debug!("✓ RESPONSE COMPLETE on conn {}", idx);
                    self.free(idx);
                    completed += 1;
                }
    ...
    fn read_from(
        ...
        loop {
            match parser(&slot.buffer) {
                ...
                None => {
                    warn!("partial buffer ({} bytes)", slot.buffer.len());
                ...
                RequestOpcode::Request => {
                    hist_request
                        .record(
                            slot.recv_time
                                .unwrap()
                                .duration_since(slot.send_time.unwrap())
                                .as_millis() as u64,
                        )
                        .unwrap();
                }
