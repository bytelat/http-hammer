...
use types::{ MsgRequest, MsgResponse, RuntimeSettings, Config, CliOptions, WorkerStats, WorkerConfig};
...
*/
    ...
    for (i, node) in cfg.upstreams.iter().enumerate() {
        println!("Processing upstream {}: {}", i, node);
        let url = build_url(node, &cfg.upstream_path);
        println!("Built URL: {}", url);
        cfg.upstream_url.push(url);
    }
//    println!("Upstream URLs: {:?}", cfg.upstream_url);
   // let concurrency = cfg.concurrency;


//    println!("Final configuration: {:?}", cfg);
...
fn main() -> anyhow::Result<()> {
   ...
   // Create the shared runtime settings
    ...
    for worker_id in 0..cfg.concurrency {
        ...
        let (tx_resp, rx_resp) = unbounded::<MsgResponse>();
    ...
    let ui_handle = ui::start_ui_thread(rx_stats,
                                                        stats.clone(), 
                                                        settings.clone(), 
                                                        //cli_opts.clone(), 
                                                        Arc::clone(&cfg));
    let injector_handle = injector::start_injector_thread(req_senders.clone(), 
